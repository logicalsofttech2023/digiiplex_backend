...
const transcodeVideoVariant = (
  inputPath: string,
  outputDir: string,
  rendition: (typeof videoRenditions)[number],
) =>
...
const transcodeAudioVariant = (
  inputPath: string,
  outputDir: string,
  rendition: (typeof audioRenditions)[number],
) =>
  new Promise<void>((resolve, reject) => {
    ensureDir(outputDir);
    ffmpeg(inputPath)
...
const processVideoJob = async (
  ...
  const masterPlaylistUrl = await uploadFolderToS3(
    tempDir,
    processingPrefix,
    async (uploaded, total) => {
      await updateProgress(
        "uploading",
        Math.round(85 + (uploaded / total) * 13),
        {
...
const processAudioJob = async (
  ...
  const masterPlaylistUrl = await uploadFolderToS3(
    ...
    async (uploaded, total) => {
      await updateProgress(
        "uploading",
        Math.round(80 + (uploaded / total) * 18),
        {
          uploadedFiles: uploaded,
          totalFiles: total,
          message: `Uploading (${uploaded}/${total})...`,
...
const worker = new Worker(
  ...
  async (job) => {
    ...
    const kind = (contentKind ?? "MOVIE") as ContentKind;
    ...
    try {
      ...
      let masterPlaylistUrl = "";
      let usedRenditions: readonly any[] = [];
      ...
      if (!isMusic) {
        ...
        await db.insert(videoVariants).values(
          (usedRenditions as typeof videoRenditions).map((r) => ({
      ...
      if (assetRole === "MAIN") {
        ...
        if (existingVideo) {
          const existingTrailerAsset = await db
            ...
            .where(
              and(
                eq(videoAssets.videoId, existingVideo.id),
                eq(videoAssets.assetType, "TRAILER"),
              ),
            )
            .limit(1)
            .then((res) => res[0] ?? null);
          ...
          if (existingTrailerAsset) {
            await db
              ...
              .set({
                ...
                drmEnabled: false,
                availableQualities: (
                  usedRenditions as typeof videoRenditions
                ).map((r) => r.label),
                updatedAt: new Date(),
              })
