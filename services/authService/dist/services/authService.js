...
export const AuthService = {
    ...
    async verifyOtp(phone, otp) {
        const user = await db.query.authUsers.findFirst({
            where: and(eq(authUsers.phone, phone), eq(authUsers.isVerified, false), eq(authUsers.isDeleted, false)),
        });
        if (!user) {
    ...
    async createProfileWithDetails(userId, data) {
    ...
    async selectLanguages(profileId, languageIds) {
        ...
        await db.insert(profileLanguages).values(languageIds.map((languageId) => ({
            profileId,
            languageId,
        })));
    ...
    async selectGenres(profileId, genreIds) {
        // Check if profile exists
        const profile = await db.query.profiles.findFirst({
            where: eq(profiles.id, profileId),
        });
        ...
        // Insert new genres
        await db.insert(profileGenres).values(genreIds.map((genreId) => ({
            profileId,
            genreId,
        })));
    ...
    async getUser(userId) {
        const user = await db.query.authUsers.findFirst({
            where: and(eq(authUsers.id, userId), eq(authUsers.isDeleted, false)),
            with: {
                profiles: {
                    with: {
                        profileLanguages: true,
