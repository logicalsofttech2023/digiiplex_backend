...
export const generateOTP = asyncHandler(async (req, res) => {
    ...
    return res
        ...
        .json(new ApiResponse(HTTP_STATUS.OK, "OTP generated successfully", { otp }));
});
// ================= OTP VERIFY =================
export const verifyOTP = asyncHandler(async (req, res) => {
    ...
    const [user] = await db
        .select()
        .from(Users)
        .where(and(eq(Users.phone, phone), eq(Users.otp, otp), gt(Users.expiresAt, new Date()), eq(Users.isDeleted, false)))
        .limit(1);
    if (!user) {
...
export const getProfiles = asyncHandler(async (req, res) => {
    ...
    return res.status(HTTP_STATUS.OK).json(new ApiResponse(HTTP_STATUS.OK, "Profiles fetched successfully", {
