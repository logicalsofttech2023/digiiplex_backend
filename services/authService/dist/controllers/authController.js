...
export const verifyOtp = asyncHandler(async (req, res) => {
    ...
    return res.json(new ApiResponse(200, "OTP verified successfully", { token, user }));
});
// ✅ 3. Create Profile with User Details (fullName, email, dob, profileName, profileImg)
export const createProfileWithDetails = asyncHandler(async (req, res) => {
    if (!req.user) {
    ...
    if (!fullName || !profileName) {
        throw new ApiError(HTTP_STATUS.BAD_REQUEST, "fullName and profileName are required");
    }
    ...
    return res.status(HTTP_STATUS.CREATED).json(new ApiResponse(HTTP_STATUS.CREATED, "Profile created successfully", result));
});
...
export const selectLanguages = asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ApiError(HTTP_STATUS.UNAUTHORIZED, "Unauthorized");
    }
    const { profileId, languageIds } = req.body;
    if (!profileId || !languageIds?.length) {
        throw new ApiError(HTTP_STATUS.BAD_REQUEST, "profileId and languageIds are required");
    }
...
export const selectGenres = asyncHandler(async (req, res) => {
    if (!req.user) {
        throw new ApiError(HTTP_STATUS.UNAUTHORIZED, "Unauthorized");
    }
    const { profileId, genreIds } = req.body;
    if (!profileId || !genreIds?.length) {
        throw new ApiError(HTTP_STATUS.BAD_REQUEST, "profileId and genreIds are required");
    }
...
export const deleteProfile = asyncHandler(async (req, res) => {
    ...
    let { profileId } = req.params;
    if (Array.isArray(profileId))
        profileId = profileId[0]; // agar array aaya toh pehla use karo
    if (!profileId) {
        throw new ApiError(HTTP_STATUS.BAD_REQUEST, "profileId is required");
    }
