...
export const rotateKey = async (_req: Request, res: Response) => {
  const key = await createNewKey();
  res.json({ message: "Key rotated", kid: key.kid });
};

export const publicKeys = async (_req: Request, res: Response) => {
  const keys = await getPublicKeys();
  res.json({ keys });
};

export const activeKey = async (_req: Request, res: Response) => {
