import { pgSchema, uuid, text, boolean, timestamp, integer, bigint, real, jsonb, index, uniqueIndex, pgEnum, varchar, date, inet, } from "drizzle-orm/pg-core";
...
export const uploads = uploadService.table("uploads", {
    ...
    creatorStatusIdx: index("idx_uploads_creator_status").on(t.creatorId, t.status),
    genreIdx: index("idx_uploads_genre_id").on(t.genreId),
    languageIdx: index("idx_uploads_language_id").on(t.languageId),
}));
// ──────────── UPLOAD ASSETS ────────────
export const uploadAssets = uploadService.table("upload_assets", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
    ...
    uniqueAssetPerUpload: uniqueIndex("unique_asset_per_upload").on(t.uploadId, t.assetRole, t.language),
}));
// ──────────── UPLOAD SESSIONS ────────────
export const uploadSessions = uploadService.table("upload_sessions", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
...
// ──────────── ASSET AUDIT LOG ────────────
export const assetAuditLog = uploadService.table("asset_audit_log", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
    ...
    cleanupIdx: index("idx_asset_audit_cleanup").on(t.cleanupStatus, t.cleanupScheduledAt),
...
// ──────────── PROCESSING JOBS ────────────
export const processingJobs = transcoderService.table("processing_jobs", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
...
// ──────────── VIDEO VARIANTS ────────────
export const videoVariants = transcoderService.table("video_variants", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
...
// ──────────── TRANSCODING PRESETS ────────────
export const transcodingPresets = transcoderService.table("transcoding_presets", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
...
// ──────────── THUMBNAILS ────────────
export const thumbnails = transcoderService.table("thumbnails", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
    processingJobId: uuid("processing_job_id")
...
// ──────────── QUEUE METRICS ────────────
export const queueMetrics = transcoderService.table("queue_metrics", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
...
// ──────────── VIDEOS ────────────
export const videos = streamingService.table("videos", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
...
// ──────────── VIDEO ASSETS ────────────
export const videoAssets = streamingService.table("video_assets", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
    ...
    uniqueVideoAsset: uniqueIndex("unique_video_asset").on(t.videoId, t.assetType, t.language),
}));
// ──────────── SUBTITLE TRACKS ────────────
export const subtitleTracks = streamingService.table("subtitle_tracks", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
    ...
    uniqueSubtitle: uniqueIndex("unique_subtitle_per_language").on(t.videoAssetId, t.language),
}));
// ──────────── AUDIO TRACKS ────────────
export const audioTracks = streamingService.table("audio_tracks", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
    videoAssetId: uuid("video_asset_id")
    ...
    uniqueAudio: uniqueIndex("unique_audio_per_language").on(t.videoAssetId, t.language),
}));
// ──────────── WATCH HISTORY ────────────
// Note: Partitioned by watched_at — partitions must be created manually in DB migrations
export const watchHistory = streamingService.table("watch_history", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
    userId: uuid("user_id")
    ...
    uniqueWatch: uniqueIndex("unique_watch_per_user").on(t.userId, t.videoId, t.videoAssetId),
}));
// ──────────── STREAMING SESSIONS ────────────
export const streamingSessions = streamingService.table("streaming_sessions", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
    userId: uuid("user_id")
...
// ──────────── CONTENT ANALYTICS ────────────
export const contentAnalytics = streamingService.table("content_analytics", {
    id: uuid("id")
        .default(sql `gen_random_uuid()`)
        .primaryKey(),
    videoId: uuid("video_id")
